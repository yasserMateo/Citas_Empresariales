...
export class ReferenciaEmpleadoListComponent {
  ...
  constructor(private referenciaSrv: ReferenciaEmpleadoService) {}
  ...
  openDialog(): void {
    this.dialog.open(ReferenciaEmpleadoFormComponent, {
      width: '480px'
    });
  }
  ...
  editReferencia(item: referencia_empleado): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.dataSource.data = this.dataSource.data.filter(p => p.idreferenciaEmpleado !== item.idreferenciaEmpleado);
        this.referenciaSrv.getReferencia(item.idreferenciaEmpleado).subscribe({
          next: (fresh) => {
            this.dataSource.data.push(fresh);
  ...
  deleteReferencia(idRefenciaEmpleado: number): void {
    this.referenciaSrv.deleteReferencia(idRefenciaEmpleado).subscribe({
      next: () => {
        this.dataSource.data = this.dataSource.data.filter(p => p.idreferenciaEmpleado !== idRefenciaEmpleado);
      },
      error: (err) => console.error(err)
    });
  }
